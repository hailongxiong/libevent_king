...
void get_post_message(char *buf, struct evhttp_request *req)
{
    ...
    else
    {
        ...
        printf("====line:%d,post len:%zu, copy_len:%zu\n",__LINE__,post_size,copy_len);
...
void http_handler_testpost_msg(struct evhttp_request *req,void *arg)
{
    ...
    else
    {
        ...
        cJSON *request_json = cJSON_Parse(buf);
        char *test = NULL;
        printf("test data:%s\n",test = cJSON_Print(request_json));

        // parse request json
        int test_size = cJSON_GetArraySize(request_json);
        printf("test data size:%d\n",test_size);

        for(int i=0; i<cJSON_GetArraySize(request_json); i++)
        {
            cJSON * item = cJSON_GetArrayItem(request_json, i);
	...
	char *buf_test = cJSON_Print(json);

    evbuffer_add_printf(retbuff, "%s", buf_test);
    evhttp_send_reply(req,HTTP_OK,"Client",retbuff);
    evbuffer_free(retbuff);
}
...
int main()
{
    ...
    const char *http_addr = "192.168.0.3";
